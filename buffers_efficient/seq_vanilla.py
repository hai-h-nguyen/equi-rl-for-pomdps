...
class SeqBuffer:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._expert_masks = np.zeros((max_replay_buffer_size, 1), dtype=np.uint8)
    ...
    def _sample_data(self, indices, next_indices):
        return dict(
            ...
            exp_msk=self._expert_masks[indices]
        )
...
if __name__ == "__main__":
    ...
    sampled_seq_len = 7
    baseline = 0.0
    buffer = SeqReplayBufferEfficient(
        buffer_size, obs_dim, act_dim, sampled_seq_len, baseline
    )
