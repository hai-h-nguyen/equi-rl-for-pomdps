...
class CloseLoopEnv(BaseEnv):
  ...
  def getGripperImg(self, gripper_state=None, gripper_rz=None):
    ...
    if self.view_type in ['camera_center_z_height', 'camera_center_z']:
      ...
      gripper_pos_xy = self.robot._getEndEffectorPosition()[:2]

      ratio_x = (gripper_pos_xy[0] - self.workspace[0, 0]) / (self.workspace[0, 1] - self.workspace[0, 0])
      ratio_y = (gripper_pos_xy[1] - self.workspace[1, 0]) / (self.workspace[1, 1] - self.workspace[1, 0])

      pixel_x = int(ratio_x*self.heightmap_size)
      pixel_y = int(ratio_y*self.heightmap_size)
    
    else:
      pixel_x = self.heightmap_size // 2
      pixel_y = self.heightmap_size // 2

    im[int(pixel_x - d // 2 - gripper_half_size):int(pixel_x - d // 2 + gripper_half_size), int(pixel_y - gripper_half_size):int(pixel_y + gripper_half_size)] = 1
    im[int(pixel_x + d // 2 - gripper_half_size):int(pixel_x + d // 2 + gripper_half_size), int(pixel_y - gripper_half_size):int(pixel_y + gripper_half_size)] = 1
