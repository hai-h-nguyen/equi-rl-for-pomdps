...
from helping_hands_rl_envs.planners.close_loop_pomdp_drawer_opening_planner import CloseLoopPomdpDrawerOpeningPlanner
from helping_hands_rl_envs.pybullet.equipments.drawer import Drawer

class CloseLoopPomdpDrawerOpeningEnv(CloseLoopEnv):
  ...
  def initialize(self):
    super().initialize()
    self.drawer.initialize((self.workspace[0].mean(), self.workspace[1].mean(), 0), pb.getQuaternionFromEuler((0, 0, 0)), 0.15)
    self.locked_drawer.initialize((self.workspace[0].mean(), self.workspace[1].mean(), 0), pb.getQuaternionFromEuler((0, 0, 0)), 0.15)
  ...
  def reset(self, target_obj_idx, noise=False):
    ...
    pos1 = pos.copy()
    pos[0] -= 0.12*np.sin(self.drawer_rot)
    pos[1] += 0.12*np.cos(self.drawer_rot)
...
def createCloseLoopPomdpDrawerOpeningEnv(config):
  return CloseLoopPomdpDrawerOpeningEnv(config)
...
if __name__ == '__main__':
  ...
  env_config['seed'] = 1
  env = CloseLoopPomdpDrawerOpeningEnv(env_config)
  planner = CloseLoopPomdpDrawerOpeningPlanner(env, planner_config)
