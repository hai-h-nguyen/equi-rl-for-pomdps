...
class CloseLoopPomdpBlockStackingEnv(CloseLoopEnv):
...
def createCloseLoopPomdpBlockStackingEnv(config):
  return CloseLoopPomdpBlockStackingEnv(config)
...
if __name__ == '__main__':
  ...
  env_config = {'workspace': workspace, 'max_steps': 100, 'obs_size': 128, 'render': True, 'fast_mode': True,
                'seed': 2, 'action_sequence': 'pxyzr', 'num_objects': 2, 'random_orientation': True,
  ...
  env_config['seed'] = 1
  env = CloseLoopPomdpBlockStackingEnv(env_config)
  planner = CloseLoopPomdpBlockStackingPlanner(env, planner_config)
