...
from helping_hands_rl_envs.planners.close_loop_pomdp_block_pushing_planner import CloseLoopPomdpBlockPushingPlanner
      ...
      try:
        if not self.random_orientation:
          ...
          goal_pos = [x, y2]
            ...
            self._generateShapes(constants.CUBE, 1, random_orientation=self.random_orientation, cube_color='blue')
            self._generateShapes(constants.CUBE, 1, random_orientation=self.random_orientation, cube_color='red')
            ...
            while True:
              ...
              xy_move = np.array(self.objects[1].getPosition()[:2])
              xy_fixed = np.array(self.objects[0].getPosition()[:2])
              d_fixed2move = np.linalg.norm(xy_fixed - xy_move)
              d_fixed2goal = np.linalg.norm(xy_fixed - np.array(goal_pos))
              d_move2goal = np.linalg.norm(xy_move - np.array(goal_pos))

              # make sure the fixed block does not block the way to push the 
              # movable block to the goal area
              if d_fixed2goal + d_fixed2move - d_move2goal > 0.1:
                break
...
def createCloseLoopPomdpBlockPushingEnv(config):
  return CloseLoopPomdpBlockPushingEnv(config)
...
if __name__ == '__main__':
  ...
  env_config['seed'] = 1
  env = CloseLoopPomdpBlockPushingEnv(env_config)
  planner = CloseLoopPomdpBlockPushingPlanner(env, planner_config)
