...
from utils.helpers import augment_shift

from escnn import nn as enn


class SAC_DrQ(RLAlgorithmBase):
    name = "sac_drq"
    ...
    def build_equi_critic(hidden_sizes, group_helper, input_size=None,
        ...
        in_type = enn.FieldType(grp_act,  input_size//num_rotations
                                * group_helper.reg_repr)
        hid_type = enn.FieldType(grp_act,  hidden_sizes[0]//num_rotations
                                 * group_helper.reg_repr)
    ...
    def critic_loss(
        ...
        with torch.no_grad():
            ...
            if markov_actor:
                ...
                for _ in range(self.K):
                    k_n_observs.append(augment_shift(observs))

                for _ in range(self.M):
                    m_observs.append(augment_shift(observs))

                for aug_idx in range(self.K):
                    # (T+1, B, dim) including reaction to last obs
                    new_actions, new_log_probs, _, _ = actor(
                        prev_actions=actions,
                        rewards=rewards,
                        observs=next_observs if markov_critic
                        else k_n_observs[aug_idx],
                    )
                    k_new_actions.append(new_actions)
                    k_new_logs.append(new_log_probs)
            ...
            if markov_critic:  # (B, 1)
                ...
                for aug_idx in range(self.K):
                    next_q1, next_q2 = critic_target(
                        prev_actions=actions,
                        rewards=rewards,
                        observs=k_n_observs[aug_idx],  # augmented s'
                        current_actions=k_new_actions[aug_idx],  # augmented a'
                    )  # (T+1, B, 1)

                    min_next_q_target = torch.min(next_q1, next_q2)

                    min_next_q_target += self.alpha_entropy * (-k_new_logs[aug_idx])  # (T+1, B, 1)

                    # q_target: (T, B, 1)
                    single_q_target = rewards + (1.0 - dones) * gamma * min_next_q_target  # next q

                    q_targets.append(single_q_target)
                q_target = torch.mean(torch.stack(q_targets), dim=0)
        ...
        if markov_critic:
            ...
            q1_preds = []
            q2_preds = []
            for aug_idx in range(self.M):
                # Q(h(t), a(t)) (T, B, 1)
                q1_pred, q2_pred = critic(
                    prev_actions=actions,
                    rewards=rewards,
                    observs=m_observs[aug_idx],  # augmented s
                    current_actions=actions[1:],
                )  # (T, B, 1)
                q1_preds.append(q1_pred)
                q2_preds.append(q2_pred)

            q1_pred = torch.mean(torch.stack(q1_preds), dim=0)
            q2_pred = torch.mean(torch.stack(q2_preds), dim=0)
