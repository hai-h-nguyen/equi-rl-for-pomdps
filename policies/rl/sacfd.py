...
class SACfD(RLAlgorithmBase):
    ...
    def actor_loss(
        ...
        action_diffs = (new_actions[:-1] - actions[1:])*expert_masks[1:]
        imitation_loss = action_diffs**2
        imitation_loss = imitation_loss.mean(dim=-1, keepdim=True)
        return policy_loss + imitation_loss, log_probs
